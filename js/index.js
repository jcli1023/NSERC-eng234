	...
	*/
	...
	function drawBorder() {
		if (!isPaused) {
			...
			//var path;
			function onMouseDown(event) {
				currentPathBorder = new Path();
				currentPathBorder.name = "currentPathBorder";
				currentPathBorder.strokeColor = 'red';
				currentPathBorder.add(event.point);
			}

			borderTrackTool1 = new Tool();
			borderTrackTool1.onMouseDown = onMouseDown;

			borderTrackTool1.onMouseDrag = function(event) {
				currentPathBorder.add(event.point);
			}
			...
			document.getElementById("test2").innerHTML = "pathBorder: " + pathBorder.exportJSON({
				asString: true
			});

			if (typeof currentPathBorder !== 'undefined')
			//			document.getElementById("test3").innerHTML = "currentPathBorder: "+currentPathBorder.segments.toString();
				document.getElementById("test3").innerHTML = "currentPathBorder: " + currentPathBorder.exportJSON({
				asString: true
			});
